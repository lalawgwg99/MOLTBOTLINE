...
所有機器人寫的檔案、筆記、程式碼，都會自動存放在專案的 **`data/`** 資料夾內。

### 6. 全局記憶庫 (Global Memory)

這是群組共用的大腦。

- **寫入**：任何人說「幫我記下來...」，Moltbot 就會存檔。
- **回憶**：任何人問「之前那個會員系統的代碼在哪？」，Moltbot 會自動去翻舊檔案回答您。
- **位置**：記憶檔案存在 `data/` 資料夾，永遠不會忘記。
